...
export class InitAll1749127025953 implements MigrationInterface {
    name = 'InitAll1749127025953'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "agents" ("id" SERIAL NOT NULL, "agent_license_number" character varying(30) NOT NULL, "userId" integer NOT NULL, "agencyId" integer, "agencyAgencyId" integer, CONSTRAINT "PK_9c653f28ae19c5884d5baf6a1d9" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "lessors" ("id" SERIAL NOT NULL, "userId" integer NOT NULL, CONSTRAINT "PK_9ea207ec8c61842e339bacc76c2" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TYPE "public"."properties_property_status_enum" AS ENUM('available', 'contracted', 'terminated')`);
        await queryRunner.query(`CREATE TABLE "properties" ("property_id" integer NOT NULL, "property_number" SERIAL NOT NULL, "property_status" "public"."properties_property_status_enum" NOT NULL DEFAULT 'available', "property_headline" character varying(100) NOT NULL, "property_address_lot" character varying(255) NOT NULL, "property_building_usage" character varying(50) NOT NULL, "property_lease_space" character varying(100) NOT NULL, "contract_lease_area" double precision NOT NULL, "property_lease_type" character varying(20) NOT NULL, "property_deposit_price" bigint NOT NULL, "property_monthly_rent_price" bigint NOT NULL, "agentId" integer NOT NULL, "lessorId" integer NOT NULL, CONSTRAINT "UQ_d007bf85f0709b85fc29bc9b6e3" UNIQUE ("property_address_lot", "property_lease_space"), CONSTRAINT "PK_b3fea131924b6a50785b0f4ce6d" PRIMARY KEY ("property_id"))`);
        await queryRunner.query(`CREATE TABLE "signatures" ("signature_id" SERIAL NOT NULL, "signature_value" character varying(512) NOT NULL, "signed_message" text NOT NULL, "signed_at" TIMESTAMP NOT NULL DEFAULT now(), "contractContractId" integer, "userId" integer, CONSTRAINT "PK_99e02b7c4f06e3e36e6a0cd5a27" PRIMARY KEY ("signature_id"))`);
        await queryRunner.query(`CREATE TABLE "payments" ("payment_id" SERIAL NOT NULL, "payment_type" character varying(20) NOT NULL, "payment_amount" bigint NOT NULL, "payment_due_date" date, "payment_actual_date" date, "payment_method" character varying(30) NOT NULL, "pg_token" character varying, "payment_status" character varying(20), "payment_hash" character varying(255), "payment_blockchain_tx_hash" character varying(255), "payment_blockchain_status" character varying(20), "payment_created_at" TIMESTAMP NOT NULL DEFAULT now(), "payment_session_url" character varying(500), "contractContractId" integer, CONSTRAINT "PK_8866a3cfff96b8e17c2b204aae0" PRIMARY KEY ("payment_id"))`);
        await queryRunner.query(`CREATE TYPE "public"."contracts_contract_status_enum" AS ENUM('PENDING', 'REQUESTED', 'DOWN_PAYMENT_PENDING', 'DOWN_PAYMENT_PAID', 'BALANCE_PAYMENT_PENDING', 'BALANCE_PAYMENT_PAID', 'DEPOSIT_RETURNED', 'TERMINATED')`);
        await queryRunner.query(`CREATE TABLE "contracts" ("contract_id" SERIAL NOT NULL, "contract_status" "public"."contracts_contract_status_enum", "contract_created_at" TIMESTAMP NOT NULL DEFAULT now(), "contract_expired_at" TIMESTAMP, "contract_termination_reason" character varying(50), "contract_hash" character varying(255), "contract_blockchain_id" integer, "contract_address" character varying(42), "contract_blockchain_tx_hash" character varying(255), "contract_blockchain_status" character varying(20) NOT NULL, "propertyPropertyId" integer, "lesseeId" integer NOT NULL, "lessorId" integer, "agentId" integer, "previousContractContractId" integer, CONSTRAINT "PK_d4c091e72433a7125d9158170e7" PRIMARY KEY ("contract_id"))`);
        ...
        await queryRunner.query(`CREATE TYPE "public"."notifications_notification_type_enum" AS ENUM('CONTRACT_REQUEST', 'CONTRACT_CREATION_REQUEST', 'CONTRACT_CREATION_READY', 'CONTRACT_DOWN_PAYMENT_REQUEST', 'CONTRACT_DOWN_PAYMENT_CONFIRMED', 'CONTRACT_FINAL_PAYMENT_REQUEST', 'CONTRACT_FINAL_PAYMENT_CONFIRMED', 'CONTRACT_DEPOSIT_RETURNED')`);
        ...
        await queryRunner.query(`CREATE TYPE "public"."contract_requests_status_enum" AS ENUM('PENDING', 'APPROVED', 'LESSOR_ACCEPTED', 'AGENT_ACCEPTED', 'ALL_ACCEPTED', 'REJECTED')`);
        await queryRunner.query(`CREATE TABLE "contract_requests" ("id" SERIAL NOT NULL, "status" "public"."contract_requests_status_enum" NOT NULL DEFAULT 'PENDING', "lessorAccepted" boolean NOT NULL DEFAULT false, "agentAccepted" boolean NOT NULL DEFAULT false, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "propertyPropertyId" integer NOT NULL, "lesseeId" integer NOT NULL, "lessorId" integer NOT NULL, "agentId" integer NOT NULL, CONSTRAINT "PK_74f2498ebadff8d03b0eee19617" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "contracts_details" ("id" SERIAL NOT NULL, "contract_property_location" character varying(255) NOT NULL, "contract_land_category" character varying(50) NOT NULL, "contract_land_area" double precision NOT NULL, "contract_building_structure" character varying(100) NOT NULL, "contract_building_usage" character varying(50) NOT NULL, "contract_building_area" double precision NOT NULL, "contract_lease_space" character varying(100) NOT NULL, "contract_lease_area" double precision NOT NULL, "contract_deposit_amount" bigint NOT NULL, "contract_deposit_korean" character varying(255) NOT NULL, "contract_down_payment" bigint NOT NULL, "contract_intermediate_payment" bigint NOT NULL, "contract_intermediate_payment_date" date NOT NULL, "contract_balance_payment" bigint NOT NULL, "contract_balance_payment_date" date NOT NULL, "contract_monthly_rent" bigint NOT NULL, "contract_rent_payment_day" integer NOT NULL, "contract_lease_start_date" date NOT NULL, "contract_lease_end_date" date NOT NULL, "contract_duration_months" integer NOT NULL, "special_terms" text, "contract_written_at" date NOT NULL, "contract_id" integer, CONSTRAINT "REL_7d466cf69b535a6d1556fb1a19" UNIQUE ("contract_id"), CONSTRAINT "PK_4742dff7af53d97855988aa0997" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`ALTER TABLE "contract_requests" ADD CONSTRAINT "FK_e3464a21d948a664e20bbc04d7a" FOREIGN KEY ("lessorId") REFERENCES "lessors"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "contract_requests" ADD CONSTRAINT "FK_c8468cad40f2e793e03890e5c38" FOREIGN KEY ("agentId") REFERENCES "agents"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "contract_requests" DROP CONSTRAINT "FK_c8468cad40f2e793e03890e5c38"`);
        await queryRunner.query(`ALTER TABLE "contract_requests" DROP CONSTRAINT "FK_e3464a21d948a664e20bbc04d7a"`);
        ...
        await queryRunner.query(`DROP TYPE "public"."contracts_contract_status_enum"`);
